...
def login():
	banner()
	x=f"{P2}Hello Users :)\n{P2}Hope that you will like this tool. {P2}Use Fresh cookies for login. {P2}Enjoy !!"
	vprint(panel(x,style=f"{H3}"))
	print("")
	x=f"{P2}[01] Login with cookie\n{P2}[02] Report bug Problem\n{P2}[{M2}00{P2}] exit "
	vprint(panel(x,style=f"{H3}"))
	cukuf = input(f" {P}[{H}â€¢{P}] Select : {H}")
	if cukuf in ["help","Help","HELP"]:
		print("")
		x=f"{P2}Facebook *--> {H2}https://www.facebook.com/M4F1A.BOY.S14M.OK3 {P2}harap chat klo ada kepentingan yang mau disampaikan ke author dmf\nini klo gak bisa diarahin ke Tool Owner yakk"
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}sedang diarahkan ke Telegram author"
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://www.facebook.com/M4F1A.BOY.S14M.OK3?text=bang+cara+pake+script+abang+kek+mana?')
		...
		print("")
		x=f"{P2}Tool Owner *--> {H2}https://www.facebook.com/M4F1A.BOY.S14M.OK3 {P2}Send me message if you having any issues to run this command..."
		vprint(panel(x,style=f"{H3}"))
		print("")
		x=f"{P2}That's All Bye Guys..."
		vprint(panel(x,style=f"{H3}"))
		os.system('xdg-open https://www.facebook.com/M4F1A.BOY.S14M.OK3?text=bang+script+mu+itu+ada+yang+error!!')
		input(f" {P}[{H}â€¢{P}] Press Enter..")
		login()
	elif cukuf in ["0","00"]:
		exit()
	else:
